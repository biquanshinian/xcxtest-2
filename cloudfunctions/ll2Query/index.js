...
function noteLl2Request(source) {
  try {
    ...
    db.collection(TIMELINE_CACHE_COL).doc(LL2_USAGE_DOC).set({
      data: {
        hourUtc: bucket,
    ...
    if (!authed && _ll2UsageCount >= 10) {
      console.warn('[LL2] anonymous hour usage high:', _ll2UsageCount, 'bucket=', bucket, '— set LL2_API_TOKEN in cloud env')
    }
  } catch (e) {}
}
...
async function resolveAutoStarshipLaunch() {
  ...
  if (_starshipMem.launchId && now - _starshipMem.ts < STARSHIP_CACHE_TTL_MS) {
    return { launchId: _starshipMem.launchId, launchName: _starshipMem.launchName, net: _starshipMem.net, source: _starshipMem.source || '', cached: true }
  }
  ...
  try {
    ...
    if (c && c.launchId && c.updatedAtMs && (now - c.updatedAtMs) < STARSHIP_DB_CACHE_TTL) {
      _starshipMem = { ts: now, launchId: c.launchId, launchName: c.launchName || '', net: c.net || '', source: c.source || '' }
      return { launchId: c.launchId, launchName: c.launchName || '', net: c.net || '', source: c.source || '', cached: true }
    }
  } catch (e) {}

  const baseQs = ['format=json', 'mode=list', 'rocket__configuration__name=' + encodeURIComponent('Starship'), 'limit=30']

  const upcomingUrl = LAUNCH_LIBRARY_API + '/launches/upcoming/?' + baseQs.concat(['ordering=' + encodeURIComponent('net')]).join('&')
  ...
  if (picked) {
    ...
    const prevUrl = LAUNCH_LIBRARY_API + '/launches/previous/?' + baseQs.concat(['ordering=' + encodeURIComponent('-net')]).join('&')
  ...
  // 写入数据库缓存
  try {
    await db.collection(TIMELINE_CACHE_COL).doc('_starship_resolve_cache').set({
...
async function resolveLaunchIdForLl2Progress(event) {
  ...
  if (manual) {
    return { launchId: manual, autoResolved: false, resolvedSource: '', resolvedLaunchName: '', resolvedNet: '', resolvedFromCache: false }
  }
  if (!autoStarship) {
    return { launchId: '', autoResolved: false, resolvedSource: '', resolvedLaunchName: '', resolvedNet: '', resolvedFromCache: false, error: 'missing_launch_id' }
  }
  try {
    const r = await resolveAutoStarshipLaunch()
    if (!r.launchId) {
      return { launchId: '', autoResolved: true, resolvedSource: '', resolvedLaunchName: '', resolvedNet: '', resolvedFromCache: false, error: 'no_starship_launch' }
    }
    return { launchId: r.launchId, autoResolved: true, resolvedSource: r.source || '', resolvedLaunchName: r.launchName || '', resolvedNet: r.net || '', resolvedFromCache: !!r.cached }
  } catch (e) {
    return { launchId: '', autoResolved: true, resolvedSource: '', resolvedLaunchName: '', resolvedNet: '', resolvedFromCache: false, error: e.message || 'resolve_starship_launch_failed' }
  }
}
  ...
  try {
    ...
    if (!launchId) {
      return {
        ...
        list: [], launchId: '', autoResolved: resolved.autoResolved, resolvedSource: resolved.resolvedSource, resolvedLaunchName: resolved.resolvedLaunchName,
        timestamp: Date.now(), elapsed: Date.now() - startTime
      }
    }
    ...
    try {
      if (!forceRefresh) {
        ...
        if (list && list.length && age < UPDATES_CACHE_TTL_COLD) {
          ...
          return {
            success: true, launchId, autoResolved: resolved.autoResolved, resolvedSource: resolved.resolvedSource, resolvedLaunchName: resolved.resolvedLaunchName,
            totalCount: cached.totalCount || list.length, list,
    ...
    const q = ['format=json', 'launch=' + encodeURIComponent(launchId), 'ordering=' + encodeURIComponent('-created_on'), 'limit=' + encodeURIComponent(String(limit))].join('&')
    ...
    // 写入缓存
    try {
      await db.collection(TIMELINE_CACHE_COL).doc(updatesCacheId).set({
        data: { data: list, totalCount: typeof apiData.count === 'number' ? apiData.count : list.length, updatedAtMs: Date.now() }
      })
    } catch (e) {}
    ...
    return {
      success: true, launchId, autoResolved: resolved.autoResolved, resolvedSource: resolved.resolvedSource, resolvedLaunchName: resolved.resolvedLaunchName,
      totalCount: typeof apiData.count === 'number' ? apiData.count : list.length, list,
      outcome: outcome || null,
      timestamp: Date.now(), elapsed: Date.now() - startTime
    }
  } catch (e) {
    return { success: false, error: e.message || 'fetch_launch_updates_failed', list: [], timestamp: Date.now(), elapsed: Date.now() - startTime }
  }
}
  ...
  try {
    ...
    if (!launchId) {
      return {
        success: false,
        error: resolved.error === 'no_starship_launch' ? 'no_starship_launch' : (resolved.error || 'missing_launch_id'),
        timeline: [], launchId: '', autoResolved: resolved.autoResolved, resolvedSource: resolved.resolvedSource, resolvedLaunchName: resolved.resolvedLaunchName,
        timestamp: Date.now(), elapsed: Date.now() - startTime
      }
    }
    ...
    try {
      ...
      if (cached && cached.updatedAtMs && (Date.now() - cached.updatedAtMs) < TIMELINE_CACHE_TTL) {
        return {
          success: true, launchId, autoResolved: resolved.autoResolved, resolvedSource: resolved.resolvedSource,
          resolvedLaunchName: resolved.resolvedLaunchName || cached.launchName || '',
          launchName: cached.launchName || '', net: cached.net || '',
          timeline: cached.data || [], timelineCount: (cached.data || []).length,
          fromCache: true, timestamp: Date.now(), elapsed: Date.now() - startTime
        }
      }
    } catch (e) {}
    ...
    try {
      await db.collection(TIMELINE_CACHE_COL).doc(cacheDocId).set({ data: { data: timeline, launchName, net, updatedAt: db.serverDate(), updatedAtMs: Date.now() } })
    } catch (e) {}

    return {
      success: true, launchId, autoResolved: resolved.autoResolved, resolvedSource: resolved.resolvedSource,
      resolvedLaunchName: resolved.resolvedLaunchName || launchName, launchName, net,
      timeline, timelineCount: timeline.length,
      timestamp: Date.now(), elapsed: Date.now() - startTime
    }
  } catch (e) {
    return { success: false, error: e.message || 'fetch_launch_timeline_failed', timeline: [], timestamp: Date.now(), elapsed: Date.now() - startTime }
  }
}
...
async function fetchLaunchDetailAction(event) {
  ...
  try {
    ...
    if (!apiData || !apiData.id) {
      return { success: false, error: 'LL2 详情接口未返回有效数据', timestamp: Date.now(), elapsed: Date.now() - startTime }
    }
    ...
    if (Array.isArray(apiData.updates) && apiData.updates.length) {
      try {
        const mapped = apiData.updates.map((u) => ({
          id: u.id,
          comment: String(u.comment || ''),
          infoUrl: typeof u.info_url === 'string' ? u.info_url.trim() : '',
          createdOn: u.created_on || '',
        ...
        if (mapped.length) {
          let existing = []
          try {
            const prev = await db.collection(TIMELINE_CACHE_COL).doc('updates_' + launchId).get()
          ...
          const merged = Array.from(byKey.values())
            ...
            .slice(0, 40)
          await db.collection(TIMELINE_CACHE_COL).doc('updates_' + launchId).set({
            data: {
              data: merged,
    ...
    return { success: false, error: e.message || 'fetch_launch_detail_failed', timestamp: Date.now(), elapsed: Date.now() - startTime }
  }
}

// ══════════════════════════════════════════════════════════════
// Action: fetchLaunchStatuses — 倒计时到点实时状态确认
// 一次拉取前 5 个即将发射任务的最新状态（不带 hide_recent_previous，
// 刚点火的任务仍带 In Flight/Success 状态出现在结果里）。
// LL2 限流保护（匿名档 15 次/小时/IP）：
//   - 120s 共享缓存（前端约 5 分钟复查，多数命中缓存，不额外打 LL2）
...
/**
 * 从 LL2 updates comment 推断终态（Launch success. / Launch failure. 等）。
...
function inferTerminalFromUpdateComments(list) {
  ...
  for (let i = 0; i < list.length; i++) {
    const u = list[i]
    if (!u) continue
    const c = String(u.comment || '').trim().toLowerCase().replace(/\s+/g, ' ')
    if (!c) continue
    let kind = null
    if (
...
/**
 * 详情单条终态 → recent_settled + previous slim 缓存 status 就地修正。
 * 解决：任务已进 previous 且离开 upcoming 探针后，历史列表长期卡在 In Flight。
 */
async function settleTerminalFromLaunchDetail(launch, nowMs, source) {
  ...
  await mergeTerminalRowsIntoRecentSettled([entry])
  try {
...
/** 用终态就地改 previous 云缓存里同 id 行的 status（不另打 LL2） */
async function patchPreviousCacheStatusFromTerminal(entries) {
  ...
  const patchRow = (row) => {
    ...
    const term = byId.get(String(row.id))
  ...
  if (isBatched) {
    for (let batchIdx = 0; batchIdx < 40; batchIdx++) {
      ...
      let batchDoc = null
      try { batchDoc = await col.doc(batchKey).get() } catch (e) { break }
  ...
  return { patched, cacheKey }
}

/** 把本次 live rows / updates 终态合并进 recent_settled（0 额外 LL2，供历史列表修正角标） */
async function mergeTerminalRowsIntoRecentSettled(rows) {
  ...
  let existing = []
  try {
    const doc = await db.collection(TIMELINE_CACHE_COL).doc(RECENT_SETTLED_DOC).get()
  ...
  for (let i = 0; i < existing.length; i++) {
    const e = existing[i]
    if (e && e.id && !byId.has(String(e.id))) byId.set(String(e.id), e)
  }
  ...
  try {
    await db.collection(TIMELINE_CACHE_COL).doc(RECENT_SETTLED_DOC).set({
      data: { data: merged, updatedAtMs: now }
    })
  } catch (e) {}
}
...
async function fetchLaunchStatusesAction() {
  ...
  if (Array.isArray(_liveStatusMem.rows) && now - _liveStatusMem.ts < LIVE_STATUS_CACHE_TTL) {
    return { success: true, rows: _liveStatusMem.rows, fromCache: 'mem', timestamp: now, elapsed: Date.now() - startTime }
  }

  // 2) 数据库缓存（跨实例共享）；15 分钟内的过期数据留作失败回落（更老的会污染列表实况）
  let staleRows = (Array.isArray(_liveStatusMem.rows) && now - _liveStatusMem.ts < LIVE_STATUS_STALE_MAX_MS) ? _liveStatusMem.rows : null
  try {
    ...
    if (cached && cached.updatedAtMs && Array.isArray(cached.data)) {
      ...
      if ((now - cached.updatedAtMs) < LIVE_STATUS_STALE_MAX_MS && (!staleRows || cached.updatedAtMs > _liveStatusMem.ts)) {
  ...
  try {
    const url = `${LAUNCH_LIBRARY_API}/launches/upcoming/?format=json&mode=list&limit=5&ordering=net`
    const apiData = await Promise.race([
      fetchAPI(url),
    ...
    try {
      ...
      if (existing.length) {
        ...
        for (let i = 0; i < rows.length; i++) {
          byId.set(String(rows[i].id), rows[i])
        }
        const out = []
        const seen = new Set()
        for (let i = 0; i < rows.length; i++) {
          out.push(rows[i])
          seen.add(String(rows[i].id))
        }
        for (let i = 0; i < existing.length; i++) {
          const row = existing[i]
    ...
    _liveStatusFailAt = 0
    try {
      await db.collection(TIMELINE_CACHE_COL).doc(LIVE_STATUS_CACHE_DOC).set({
        data: { data: mergedRows, updatedAtMs: Date.now() }
      })
    } catch (e) {}
    // 终态顺带写入 recent_settled，历史列表可修正角标（不另打 LL2）
    try { await mergeTerminalRowsIntoRecentSettled(rows) } catch (e) {}
    ...
    if (staleRows) {
      ...
      return { success: true, rows: staleRows, fromCache: 'stale', timestamp: Date.now(), elapsed: Date.now() - startTime }
    }
    return { success: false, error: e.message || 'fetch_launch_statuses_failed', rows: [], timestamp: Date.now(), elapsed: Date.now() - startTime }
  }
}
...
async function translateTextsAction(event) {
  ...
  const texts = raw
    .slice(0, TRANSLATE_MAX_ITEMS)
    .map((t) => String(t || '').slice(0, TRANSLATE_MAX_ITEM_CHARS))
  ...
  try {
    ...
    return { success: false, error: e.message || 'translate_failed', timestamp: Date.now(), elapsed: Date.now() - startTime }
  }
}
...
async function ensureCollections() {
  ...
  const names = ['launch_timeline_cache', 'space_devs_cache']
  for (const n of names) {
    try { await db.createCollection(n) } catch (e) {}
  }
}
