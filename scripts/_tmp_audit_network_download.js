...
check('Worker /image 不落盘', /isWorkerImageProxyUrl/.test(icon))
check('首页轮播不 downloadFile', /_isHomeCarouselUrl/.test(icon))
check('火箭后台下载禁外链', /_downloadRocketInBackground[\s\S]*!isOwnCdnUrl/.test(icon))
check('火箭配置图默认 thumb 480', /preset \|\| 'thumb'/.test(icon) && /optimizeImageUrl\(_stripImageProcessParams\(cdn\), p\)/.test(icon))
...
check('火箭未命中不猜 COS 文件名', /default\.jpg/.test(imgCfg) && /不再按字典文件名直拼 COS/.test(imgCfg))
...
check('media map 不再由客户端调 COS 列举', !/maybeInvokeRocketCosSync/.test(imgCfg) && !/syncRocketCosIndex/.test(imgCfg))
check('media map 不再 await COS 列举再拉清单', !/await maybeInvokeRocketCosSync\(\)\s*\n\s*try \{\s*\n\s*let fetchedMap/.test(imgCfg))
...
const { isOwnCdnUrl, isWorkerImageProxyUrl, proxiedImageUrl } = require('../utils/ll2-image.js')
...
check('代理 URL 识别', isWorkerImageProxyUrl(proxied))
