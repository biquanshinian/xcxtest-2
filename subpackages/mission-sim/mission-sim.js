...
function buildDiagramGeo(profile) {
  ...
  if (profile.boosterEnd === 'catch') {
    ...
    // 受控溅落：减速后落入近海回收区
    booster = ascent.concat([
      [230, 0.47, 0.60], [260, 0.43, 0.46], [310, 0.38, 0.29],
      [360, 0.345, 0.16], [420, 0.325, 0.05], [444, 0.32, 0.008]
    ])
  }
  ...
  if (profile.shipEnd === 'catch') {
    ...
    // 二级溅落点与一级溅落点拉开水平距离，避免水花/标记重叠
    ship = shipHead.concat([
      [se - 421, 0.94, 0.74], [se - 271, 0.88, 0.55], [se - 201, 0.80, 0.40], [se - 121, 0.70, 0.24],
      [se - 71, 0.63, 0.12], [se - 31, 0.595, 0.04], [se, 0.58, 0.008]
    ])
  }
  ...
  if (profile.boosterEnd === 'splash') {
    nodes.push({ x: 0.32, y: 0.02, label: '一级溅落', align: 'l', c: 'g' })
  }
  if (profile.shipEnd === 'splash') {
    nodes.push({ x: 0.80, y: 0.40, label: '黑障段', align: 'r', c: 'o' })
    nodes.push({ x: 0.58, y: 0.02, label: '二级溅落', align: 'r', c: 'b' })
  } else {
...
Page({
  ...
  onLoad: async function (options) {
    ...
    that._trailB = []
    that._trailS = []
    that._geo = buildDiagramGeo(DEFAULT_GEO_PROFILE)
    that.initUiShell()
  ...
  onShow: function () {
    if (this.data.started && !this._timer && this._mission) this._startTimer()
  },
  ...
  _begin: function (seed) {
    ...
    that._geo = buildDiagramGeo(that._mission.snapshot().profile || DEFAULT_GEO_PROFILE)
    // 保留玩家上一局选择的倍率
  ...
  _startTimer: function () {
    var that = this
    that._stopTimer()
    that._timer = setInterval(function () { that._tick() }, TICK_MS)
  },
  ...
  _tick: function () {
    ...
    var snap = this._mission.step(TICK_MS)
  ...
  _initCanvas: function (attempt) {
    ...
    attempt = attempt || 0
    if (attempt > 8) return
    that.createSelectorQuery().select('#msDiagram')
      .fields({ node: true, size: true })
      .exec(function (res) {
        if (!res || !res[0] || !res[0].node) {
          setTimeout(function () { that._initCanvas(attempt + 1) }, 200)
          return
        }
        var canvas = res[0].node
        var dpr = wx.getWindowInfo().pixelRatio || 2
        that._canvasW = res[0].width * dpr
        that._canvasH = res[0].height * dpr
        ...
        if (that._mission) that._drawDiagram(that._mission.snapshot())
      })
    that._initEngineCanvas(0)
  },

  _initEngineCanvas: function (attempt) {
    var that = this
    attempt = attempt || 0
    if (attempt > 8) return
    that.createSelectorQuery().select('#msEngines')
      .fields({ node: true, size: true })
      .exec(function (res) {
        if (!res || !res[0] || !res[0].node) {
          setTimeout(function () { that._initEngineCanvas(attempt + 1) }, 200)
          return
        }
        var canvas = res[0].node
        var dpr = wx.getWindowInfo().pixelRatio || 2
        that._engW = res[0].width * dpr
        that._engH = res[0].height * dpr
        canvas.width = that._engW
        canvas.height = that._engH
        that._engCanvas = canvas
        that._engCtx = canvas.getContext('2d')
        if (that._mission) that._drawEngines(that._mission.snapshot())
      })
  },
  ...
  _paintBase: function (ctx, w, h) {
    ...
    if (geo.profile.boosterEnd === 'splash') splashZone(0.32, 'rgba(0,255,136,0.3)')
    if (geo.profile.shipEnd === 'splash') splashZone(0.58, 'rgba(120,180,255,0.3)')
  ...
  _drawDiagram: function (snap) {
    ...
    if (geo.profile.boosterEnd === 'splash' && t >= 438 && t <= 560) {
      this._drawSplash(ctx, this._px(0.32, w), gy, (t - 438) / 20, '0,255,136')
    }
    ...
    if (geo.profile.shipEnd === 'splash' && t >= seT && t <= seT + 147) {
      this._drawSplash(ctx, this._px(0.58, w), gy, (t - seT) / 30, '120,180,255')
    }
