...
function mapSplashMediaItem(it) {
...
function normalizeItems(cfg) {
  if (!cfg) return []
  // 显式 mediaItems（含空数组）是权威池，禁止再回落顶层 mediaUrl
  if (Array.isArray(cfg.mediaItems)) {
    return cfg.mediaItems.filter((it) => it && it.mediaUrl).map(mapSplashMediaItem)
  }
  if (cfg.mediaUrl) {
    return [mapSplashMediaItem(cfg)]
  }
  return []
}
...
function splashCacheAllowsLocalFileWrite(cur) {
  if (!cur || cur.enabled === false) return false
  if (Array.isArray(cur.mediaItems) && !cur.mediaItems.length) return false
  return true
}

function pruneSplashSavedFiles(localPaths, localPath) {
  const paths = []
  if (typeof localPath === 'string' && localPath) paths.push(localPath)
  if (localPaths && typeof localPaths === 'object') {
    Object.keys(localPaths).forEach((k) => {
      const p = localPaths[k]
  ...
  try {
    ...
    if (!fs || typeof fs.removeSavedFile !== 'function') return
    const seen = {}
    paths.forEach((p) => {
      if (seen[p]) return
      seen[p] = true
      try {
        fs.removeSavedFile({ filePath: p, fail() {} })
      } catch (e) {}
    })
  } catch (e) {}
}
    ...
    wx.getFileSystemManager().saveFile({
      ...
      success: (saveRes) => {
        ...
        try {
          ...
          if (!splashCacheAllowsLocalFileWrite(cur)) return
...
function warmSplashMissionSideData(state) {
  try {
    ...
    if (logo && /^https?:\/\//i.test(logo)) {
      preloadImage(logo)
      try {
        require('../../../utils/agency-logo-cache.js').persistAgencyLogoAfterRemoteLoad(logo)
      } catch (e) {}
    }
  } catch (e) {}
...
function clearPrefetchPick(state) {
  if (!state) return
  state.picked = null
  state.resolved = null
  state.playUrl = ''
...
function persistSplashCfg(cfg, state) {
  ...
  try {
    ...
    const explicitPool = Array.isArray(cfg.mediaItems)
    const poolCleared = cfg.enabled === false || (explicitPool && !normalizeItems(cfg).length)
    const mediaItems = poolCleared ? [] : explicitPool ? cfg.mediaItems : prev.mediaItems || []
    const next = {
      ...prev,
      enabled: cfg.enabled !== false,
      mediaItems,
      lastSplashId: poolCleared ? '' : prev.lastSplashId || '',
      ...
      localPaths: poolCleared ? {} : prev.localPaths && typeof prev.localPaths === 'object' ? prev.localPaths : {},
      localPath: poolCleared ? '' : prev.localPath || '',
    ...
    if (poolCleared) {
      ...
      next.posterUrl = ''
      next.mediaType = ''
      pruneSplashSavedFiles(prev.localPaths, prev.localPath)
    }
...
function applyCloudCfg(state, cfg) {
  ...
  if (cfg.enabled === false) {
    ...
    if (state.cached) state.cached = { enabled: false, updatedAt: splashConfigUpdatedAt(cfg) }
    if (!state.consumed) state.skip = true
    clearPrefetchPick(state)
    return
  }
  const items = normalizeItems(cfg)
  if (!items.length) {
    if (Array.isArray(cfg.mediaItems) || cfg.enabled === false) {
      if (!state.consumed) state.skip = true
      clearPrefetchPick(state)
    }
    return
