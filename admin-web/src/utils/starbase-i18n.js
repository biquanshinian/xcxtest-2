/**
 * Starbase.gov 封路/海滩英文文案 → 中文（管理后台展示）
 */
...
export function translateMayorOrderBody(text) {
...
export function applyStarbaseI18n(text) {
  const raw = decodeHtmlEntities(String(text || '').trim())
  if (!raw) return ''
...
export function resolveRoadStatusDisplay(record) {
