<template>
  <el-card>
    ...
    <el-form :model="form" label-width="160px" style="max-width:760px">
      ...
      <el-form-item label="配图跳小程序">
        <el-radio-group v-model="form.imageMiniprogramLinkMode">
          <el-radio value="none">不跳转</el-radio>
          <el-radio value="first">仅第一张</el-radio>
          <el-radio value="last">仅最后一张</el-radio>
          <el-radio value="first_last">首尾两张</el-radio>
          <el-radio value="all">全部配图</el-radio>
        </el-radio-group>
        <el-text type="info" size="small" style="display:block;margin-top:6px;">
          被选中的正文配图点击进小程序（须各发稿号已关联同一小程序；配图会按凭证槽本号转存）
        </el-text>
      </el-form-item>
...
const load = async () => {
  try {
    ...
    if (!['none', 'first', 'last', 'first_last', 'all'].includes(form.imageMiniprogramLinkMode)) {
      form.imageMiniprogramLinkMode = form.linkAllImagesToMiniprogram === false ? 'none' : 'all'
    }
