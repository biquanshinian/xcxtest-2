  ...
  <div>
    ...
    <el-card v-if="activeTab !== 'claims'">
      ...
      <el-table :data="list" v-loading="loading" stripe>
        ...
        <el-table-column label="奖品类型" width="130">
          <template #default="{ row }">
            <el-tag v-if="row.prizeType === 'pro_1month'" type="warning" size="small">1个月PRO</el-tag>
            <el-tag v-else size="small">实物奖品</el-tag>
          </template>
        </el-table-column>
    ...
    <el-card v-if="activeTab === 'claims'">
      <template #header>
        <div style="display:flex;justify-content:space-between;align-items:center">
          ...
          <el-select v-model="claimStatusFilter" placeholder="全部状态" clearable style="width:140px" @change="loadClaims">
            ...
            <el-option label="已完成 / 已到账" value="completed" />
          </el-select>
        </div>
      </template>
      ...
      <el-table :data="claims" v-loading="claimsLoading" stripe>
        ...
        <el-table-column label="奖品类型" width="110">
          <template #default="{ row }">
            <el-tag v-if="isProClaim(row)" type="warning" size="small">1个月PRO</el-tag>
            <el-tag v-else size="small">实物</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="收件人" prop="name" width="100">
          <template #default="{ row }">
            <span v-if="isProClaim(row)" style="color:#999">—</span>
        ...
        <el-table-column label="地址" prop="address" min-width="200" show-overflow-tooltip>
          <template #default="{ row }">
            <span v-if="isProClaim(row)" style="color:#999">系统自动到账</span>
            <span v-else>{{ row.address || '-' }}</span>
          </template>
        </el-table-column>
        <el-table-column label="状态" width="100">
          <template #default="{ row }">
            <el-tag :type="claimStatusType(row)" size="small">{{ claimStatusLabel(row) }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="{ row }">
            <el-button v-if="row.status === 'pending' && !isProClaim(row)" size="small" type="primary" @click="onShip(row)">发货</el-button>
            <el-button v-if="!isProClaim(row) && (row.status === 'shipped' || row.status === 'completed')" size="small" @click="onEditTracking(row)">改单号</el-button>
            <el-button v-if="row.status === 'shipped' && !isProClaim(row)" size="small" type="success" @click="onComplete(row)">完成</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑里程碑' : '新增里程碑'" width="560px" destroy-on-close>
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="奖品类型">
          <el-radio-group v-model="form.prizeType" @change="onPrizeTypeChange">
            <el-radio value="physical">实物奖品</el-radio>
            <el-radio value="pro_1month">1个月PRO会员</el-radio>
          </el-radio-group>
          <div v-if="form.prizeType === 'pro_1month'" style="margin-top:6px;color:#909399;font-size:12px;line-height:1.6">
            用户达标后由系统根据云端签到 / 问答 / 竞猜记录自动判定并赠送 30 天 PRO，无需白名单、无需用户提供 ID、无需运营发货。
          </div>
        </el-form-item>
        ...
        <el-form-item label="奖品描述">
          <el-input
            ...
            type="textarea"
            :rows="2"
            :placeholder="form.prizeType === 'pro_1month' ? '例：1个月PRO会员，达标后系统自动到账' : '例：T恤一件，运费到付'"
          />
        </el-form-item>
        ...
        <template v-if="form.prizeType !== 'pro_1month'">
...
function isProClaim(row) {
  return row?.prizeType === 'pro_1month' || row?.grantKind === 'pro_1month'
}

function claimStatusLabel(row) {
  if (isProClaim(row)) return row?.status === 'completed' ? '已到账' : '发放中'
  if (row?.status === 'pending') return '待发货'
  if (row?.status === 'shipped') return '已发货'
  return '已完成'
}

function claimStatusType(row) {
  if (isProClaim(row)) return 'success'
  if (row?.status === 'pending') return 'warning'
  if (row?.status === 'shipped') return ''
  return 'success'
}

function onPrizeTypeChange(v) {
  if (v === 'pro_1month' && !form.description) {
    form.description = '1个月PRO会员，达标后系统自动到账'
  }
}
...
function openDialog(row) {
  ...
  form.prizeType = row?.prizeType === 'pro_1month' ? 'pro_1month' : 'physical'
...
async function onSave() {
  ...
  try {
    const payload = { ...form }
    payload.prizeType = form.prizeType === 'pro_1month' ? 'pro_1month' : 'physical'
    payload.customOptions = payload.prizeType === 'pro_1month'
      ? []
      : (form.customOptions || [])
    ...
    if (payload.prizeType === 'pro_1month') payload.customNote = ''
    if (payload.prizeType === 'pro_1month' && !payload.description) {
      payload.description = '1个月PRO会员，达标后系统自动到账'
    }
    if (editing.value) {
