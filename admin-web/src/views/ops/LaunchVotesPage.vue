  ...
  <div>
    ...
    <el-card style="margin-bottom:16px;">
      ...
      <div style="color:var(--t-text-muted, #86868b);font-size:12px;margin-top:4px;">
        开启后，所有未单独配置竞猜的任务都会显示对应题型。下方可为特定任务创建独立竞猜（会覆盖全局配置）。同一任务可同时开通准时和成败，互不影响。成败结算：成功/载荷部署→成功；失败/部分失败/取消等→失败。
      </div>
    </el-card>
    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑竞猜' : '新建竞猜'" width="640px">
      <el-form :model="form" label-width="120px">
        ...
        <el-form-item label="当前轮次" v-if="form.voteType !== 'outcome'">
          ...
          <span style="margin-left:8px;color:#909399;font-size:12px;">推迟后系统自动递增，也可手动调整</span>
        </el-form-item>
        <el-form-item label="历史轮次" v-if="form.voteType !== 'outcome' && form.rounds && form.rounds.length">
          <div v-for="(r, idx) in form.rounds" :key="idx" style="margin-bottom:4px;font-size:13px;color:#909399;">
