  ...
  <el-card>
    <template #header>
      <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;">
        <div>
          <div>事件更新追踪</div>
          <div class="evt-header-hint">自动同步推文按 3 天生命周期清理；手动录入的事件会保留。</div>
        </div>
    ...
    <el-table :data="list" stripe>
      ...
      <el-table-column label="媒体" width="150" align="center">
        <template #default="scope">
          <div v-if="(scope.row.mediaList || []).length" class="evt-media-cell">
            <MediaThumb
              v-for="(m, mi) in scope.row.mediaList.slice(0, 3)"
              :key="mi"
              :src="m.url"
              :type="m.type"
            ...
            />
            <span v-if="scope.row.mediaList.length > 3" class="evt-media-more">+{{ scope.row.mediaList.length - 3 }}</span>
          </div>
          <span v-else>-</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="账号" width="180">
        <template #default="scope">
          <div v-if="scope.row.source || scope.row.author" class="evt-account">
            <el-avatar v-if="scope.row.authorAvatar" :size="22" :src="scope.row.authorAvatar" />
            <div class="evt-account-text">
              <span class="evt-account-handle">{{ scope.row.source ? `@${scope.row.source}` : '手动' }}</span>
              <span v-if="scope.row.author" class="evt-account-author">{{ scope.row.author }}</span>
            </div>
            <el-link
              v-if="scope.row.tweetUrl"
              :href="scope.row.tweetUrl"
              target="_blank"
              type="primary"
              :underline="false"
              class="evt-tweet-link"
            >推文</el-link>
          </div>
          <span v-else>-</span>
        </template>
        ...
        <div class="media-upload-area">
          <div class="media-preview-grid" v-if="form.mediaList.length > 0">
            <div v-for="(media, idx) in form.mediaList" :key="idx" class="media-preview-card">
              <img v-if="media.type === 'image'" :src="media.url" class="media-thumb" title="点击查看大图" @click="previewMedia(media.url, { type: 'image', title: form.title })" />
              <video v-else-if="media.type === 'video'" :src="media.url" class="media-thumb" title="点击播放" @click="previewMedia(media.url, { type: 'video', poster: media.thumbnailUrl || '', title: form.title })" />
