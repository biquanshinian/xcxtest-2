<template>
  ...
  <el-card class="tm-card">
    ...
    <el-table :data="list" v-loading="loading" size="small">
      ...
      <el-table-column label="媒体" width="120" align="center">
        <template #default="scope">
          <div v-if="(scope.row.mediaList || []).length" class="tm-media-cell">
            <MediaThumb
              v-for="(m, mi) in scope.row.mediaList.slice(0, 2)"
              :key="mi"
              :src="m.url"
              :type="m.type"
            ...
            />
            <span v-if="scope.row.mediaList.length > 2" class="tm-media-more">+{{ scope.row.mediaList.length - 2 }}</span>
          </div>
          <span v-else>-</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="来源" width="170">
        <template #default="scope">
          <div class="tm-src-cell">
            <el-avatar v-if="scope.row.authorAvatar" :size="20" :src="scope.row.authorAvatar" />
            <span class="tm-src-author">{{ scope.row.author || '-' }}</span>
            <el-tag size="small" effect="plain" :type="scope.row.source === 'auto_sync' ? 'success' : 'info'">
              {{ scope.row.source === 'auto_sync' ? '自动' : '手动' }}
            </el-tag>
            <el-link
