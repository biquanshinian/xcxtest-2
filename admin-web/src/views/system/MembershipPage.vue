  ...
  <div class="membership-page">
    <div class="page-header">
      ...
      <div class="page-header-actions">
        ...
        <el-button :icon="Refresh" :loading="loading || skuPricesLoading" @click="refreshAll">刷新</el-button>
      </div>
    </div>
    ...
    <el-card class="section-card pro-card" shadow="never">
      <template #header>
        <div class="section-header">
          <div>
            ...
            <span class="section-hint">按套餐类型分组，组内按付款日期从新到旧</span>
          </div>
          <el-tag type="warning" effect="dark" size="small" round>{{ visibleProCount }} 人</el-tag>
        </div>
      </template>

      <div class="pro-toolbar">
        <el-radio-group v-model="proPlanFilter" size="small">
          <el-radio-button label="all">
            全部 {{ filteredProMembers.length }}
          </el-radio-button>
          <el-radio-button
            v-for="g in proPlanGroups"
            :key="g.key"
            :label="g.key"
      ...
      <el-table
        ...
        style="width:100%"
        :span-method="proSpanMethod"
        :row-class-name="proRowClassName"
        row-key="_id"
        size="small"
        @row-click="onProRowClick"
      >
        <el-table-column prop="_id" label="OpenID" min-width="220" show-overflow-tooltip>
          <template #default="{ row }">
            <div v-if="row._isGroup" class="pro-group-row">
              <el-icon class="pro-group-arrow" :class="{ 'is-collapsed': isProGroupCollapsed(row.groupKey) }">
                <ArrowDown />
              </el-icon>
              <el-tag :type="row.tagType" size="small" effect="dark">{{ row.groupLabel }}</el-tag>
              <span class="pro-group-count">{{ row.groupCount }} 人</span>
              <span v-if="row.latestPaidLabel" class="pro-group-hint">最近付款 {{ row.latestPaidLabel }}</span>
              <span class="pro-group-hint">{{ isProGroupCollapsed(row.groupKey) ? '点击展开' : '点击收起' }}</span>
            </div>
            <span v-else class="pro-openid">{{ row._id }}</span>
          </template>
        </el-table-column>
        <el-table-column label="来源" width="160">
          <template #default="{ row }">
            <template v-if="!row._isGroup">
              <el-tag v-if="row.inWhitelist && row.planGroup !== 'whitelist'" type="warning" size="small" style="margin:2px;">白名单</el-tag>
        ...
        <el-table-column label="套餐" width="100">
          <template #default="{ row }">
            <el-tag v-if="!row._isGroup && row.planLabel" :type="planTagType(row.planLabel)" size="small" effect="plain">
              {{ row.planLabel }}
            </el-tag>
            <span v-else-if="!row._isGroup" style="color:#999">-</span>
          </template>
        </el-table-column>
        <el-table-column label="付款日期" width="170">
          <template #default="{ row }">
            <span v-if="!row._isGroup">{{ row.paidAt ? formatDateMinute(row.paidAt) : '-' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="expireAt" label="到期时间" width="170">
          <template #default="{ row }">
            <template v-if="!row._isGroup">
              <span v-if="isLongTermExpire(row)" style="color:#909399;">长期</span>
              <span v-else>{{ row.expireAt ? formatDateMinute(row.expireAt) : '-' }}</span>
            </template>
          </template>
        </el-table-column>
        <el-table-column prop="purchases" label="已购产品" min-width="160">
          <template #default="{ row }">
            <template v-if="!row._isGroup">
        ...
        <el-table-column label="操作" width="110">
          <template #default="{ row }">
            <el-button v-if="!row._isGroup" size="small" link @click.stop="openGrantDialog(row._id)">续期/赠送</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    <el-card class="section-card" shadow="never">
      <template #header>
        <div class="section-header">
          <div>
            ...
            <span class="section-hint">支持按 OpenID / 状态 / 类型筛选，支持 CSV 导出。Apple 支付订单不可人工退款，请引导用户在 App Store 申请</span>
          </div>
      ...
      <el-table :data="orderPage" v-loading="orderPageLoading" stripe empty-text="暂无订单" style="width:100%">
        ...
        <el-table-column label="支付渠道" width="110">
          <template #default="{ row }">
            <el-tag size="small" :type="isApplePayOrder(row) ? 'primary' : 'success'" effect="plain">
              {{ payChannelText(row) }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="170" fixed="right">
          <template #default="{ row }">
            <el-tooltip
              v-if="row.status === 'paid' && isApplePayOrder(row)"
              content="Apple 支付不支持开发者主动退款，请引导用户在 App Store 申请"
              placement="top"
            >
              <span class="apple-refund-hint">App Store 退款</span>
            </el-tooltip>
            <el-tooltip
              v-else-if="row.status === 'paid' && !isRefundableOrder(row)"
              content="赠送 / 邀请 / 零元订单不能走微信退款"
              placement="top"
            >
              <span class="apple-refund-hint">不可退款</span>
            </el-tooltip>
            <el-button
      ...
      <template #header>
        <div class="section-header">
          <div>
            ...
            <span class="section-hint">offerId 与环境写入库后由 membership 下单签名读取（env=0 现网；env=1 沙箱，仅安卓/鸿蒙/Windows）。iOS Apple 支付仅支持现网，下单会强制 env=0 与现网 AppKey。AppKey 仍用云函数环境变量</span>
          </div>
        </div>
      </template>
      <el-form :model="vpayConfig" label-width="120px" style="max-width:560px;">
        ...
        <el-form-item label="环境">
          <el-radio-group v-model="vpayConfig.env">
            ...
            <el-radio :label="1">沙箱（env=1，仅安卓/鸿蒙/Windows）</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item>
          ...
          <template #default="{ row }">
            <div style="display:flex;align-items:center;gap:8px;">
              <el-input-number
                ...
                style="width:160px;"
              />
              <span style="color:#909399;font-size:12px;">¥ {{ (row.editPrice / 100).toFixed(2) }}</span>
            </div>
          </template>
        </el-table-column>
...
function formatDateMinute(val) {
  const s = formatDate(val)
  return s.length >= 19 ? s.slice(0, 16) : s
}

/** 到期还剩 10 年以上（永久套餐约 100 年）视为长期；白名单无到期日也视为长期 */
function isLongTermExpire(row) {
  if (!row) return false
  const ms = toMs(row.expireAt)
  if (!ms) return row.proSource === 'whitelist' || row.planGroup === 'whitelist'
  return ms - Date.now() > 10 * 365.25 * 86400000
}
...
/** planId → 套餐标签（月卡 / 年卡 / 永久 / 折扣变体） */
function planLabelOf(planId) {
  const p = String(planId || '')
  if (!p) return ''
  if (p === 'permanent_discount') return '永久5折'
  if (p.startsWith('permanent')) return '永久'
  if (p === 'yearly_discount') return '年卡6折'
...
function planGroupKeyOf(planId) {
  const p = String(planId || '')
  if (p.startsWith('permanent')) return 'permanent'
  if (p.startsWith('yearly')) return 'yearly'
  if (p === 'monthly') return 'monthly'
  if (p) return 'other'
  return ''
}

function planTagType(label) {
  const s = String(label || '')
  if (s.startsWith('永久')) return 'danger'
  if (s.startsWith('年卡')) return 'warning'
  if (s === '月卡') return 'success'
  return 'info'
}

const PLAN_GROUP_DEFS = [
  { key: 'permanent', label: '永久', tagType: 'danger' },
  { key: 'yearly', label: '年卡', tagType: 'warning' },
  { key: 'monthly', label: '月卡', tagType: 'success' },
  { key: 'other', label: '其他套餐', tagType: 'info' },
  { key: 'whitelist', label: '白名单', tagType: 'warning' },
  { key: 'product', label: '单品', tagType: 'primary' }
]

const PRO_TABLE_COL_COUNT = 7

/**
 * 从订单流水推导每个 openid 的订阅获取方式、最新套餐与最近付款时间：
...
const memberSubMeta = computed(() => {
  ...
  for (const o of orders.value) {
    if (!o || o.status !== 'paid' || !o.openid) continue
    let entry = meta.get(o.openid)
    if (!entry) {
      entry = { sources: new Set(), latestPlanId: '', latestPaidAt: 0, latestProductPaidAt: 0 }
      meta.set(o.openid, entry)
    }
    const ts = toMs(o.paidAt || o.createdAt)
    if (o.orderType === 'subscription') {
      const id = String(o._id || '')
      if (o.grantReason === 'invite_reward' || id.startsWith('I')) {
        entry.sources.add('invite')
      } else if (
        o.grantSource === 'milestone' ||
        o.milestoneId ||
        id.startsWith('MS') ||
        String(o.grantReason || '').indexOf('milestone') === 0
      ) {
      ...
      if (ts > entry.latestPaidAt) entry.latestPaidAt = ts
    } else if (o.orderType === 'product') {
      if (ts > entry.latestProductPaidAt) entry.latestProductPaidAt = ts
    }
  }
  return meta
})
...
function acqTagsOf(openid) {
  ...
  if (entry.sources.has('milestone')) tags.push({ label: '里程碑', type: 'warning' })
  return tags
}
...
const proMembers = computed(() => {
  ...
  for (const m of members.value) {
    ...
    const paid = isPaidPro(m)
    if (paid || inWhitelist || hasProducts(m)) {
      const entry = memberSubMeta.value.get(m._id)
      const planId = paid ? (m.planId || (entry && entry.latestPlanId) || '') : ''
      const planLabel = paid ? planLabelOf(planId) : ''
      let planGroup = planGroupKeyOf(planId)
      if (!planGroup) {
        if (paid) planGroup = 'other'
        else if (inWhitelist) planGroup = 'whitelist'
        else planGroup = 'product'
      }
      const proSource = paid ? 'paid' : (inWhitelist ? 'whitelist' : 'product')
      const paidAtMs = (entry && (entry.latestPaidAt || entry.latestProductPaidAt)) || toMs(m.paidAt) || 0
      list.push({
        ...m,
        inWhitelist,
        proSource,
  ...
  list.sort((a, b) => (b.paidAtMs || 0) - (a.paidAtMs || 0))
  return list
})

const proPlanFilter = ref('all')
const proSearch = ref('')
const proCollapsedGroups = ref(new Set())

const filteredProMembers = computed(() => {
  const q = String(proSearch.value || '').trim().toLowerCase()
  if (!q) return proMembers.value
  return proMembers.value.filter((m) => String(m._id || '').toLowerCase().includes(q))
})

const proPlanGroups = computed(() => {
  const buckets = Object.fromEntries(PLAN_GROUP_DEFS.map((d) => [d.key, []]))
  for (const m of filteredProMembers.value) {
    const key = buckets[m.planGroup] ? m.planGroup : 'other'
    buckets[key].push(m)
  }
  return PLAN_GROUP_DEFS.map((d) => ({
    ...d,
    members: buckets[d.key] || []
  }))
})

const proTableRows = computed(() => {
  const filter = proPlanFilter.value
  const collapsed = proCollapsedGroups.value
  const rows = []
  for (const g of proPlanGroups.value) {
    if (filter && filter !== 'all' && g.key !== filter) continue
    if (!g.members.length) continue
    const latest = g.members[0]
    rows.push({
      _isGroup: true,
      _id: `__group_${g.key}`,
      groupKey: g.key,
      groupLabel: g.label,
      groupCount: g.members.length,
      tagType: g.tagType,
      latestPaidLabel: latest && latest.paidAt ? formatDateMinute(latest.paidAt) : ''
    })
    if (!collapsed.has(g.key)) rows.push(...g.members)
  }
  return rows
})

function isProGroupCollapsed(key) {
  return proCollapsedGroups.value.has(key)
}

function onProRowClick(row) {
  if (!row || !row._isGroup) return
  const next = new Set(proCollapsedGroups.value)
  if (next.has(row.groupKey)) next.delete(row.groupKey)
  else next.add(row.groupKey)
  proCollapsedGroups.value = next
}

function proSpanMethod({ row, columnIndex }) {
  if (!row || !row._isGroup) return [1, 1]
  if (columnIndex === 0) return [1, PRO_TABLE_COL_COUNT]
  return [0, 0]
}

function proRowClassName({ row }) {
  if (row && row._isGroup) return 'pro-group-header-row'
  return ''
}

const visibleProCount = computed(() => {
  const filter = proPlanFilter.value
  return proPlanGroups.value
    .filter((g) => !filter || filter === 'all' || g.key === filter)
    .reduce((n, g) => n + g.members.length, 0)
})

watch([proSearch, proPlanGroups], () => {
  if (proPlanFilter.value === 'all') return
  const g = proPlanGroups.value.find((x) => x.key === proPlanFilter.value)
  if (!g || !g.members.length) proPlanFilter.value = 'all'
})
...
async function exportOrders() {
  try {
    ...
    const headers = ['订单号', '用户 OpenID', '类型', '支付渠道', '商品/计划', '金额(元)', '状态', '虚拟支付商品', '创建时间', '支付时间']
...
async function onRecheckPending() {
  ...
  try {
    const res = await api.recheckPendingMembershipOrders({ limit: 200 })
...
function isApplePayOrder(order) {
  ...
  return String(order.platform || '').trim().toLowerCase() === 'ios'
}
function isRefundableOrder(order) {
  if (!order || order.status !== 'paid') return false
  if (isApplePayOrder(order)) return false
  if (!Number(order.amount)) return false
  const id = String(order._id || '')
  if (order.grantBy || id.startsWith('G') || id.startsWith('I') || id.startsWith('MS')) return false
  return true
}
function payChannelText(order) {
  return isApplePayOrder(order) ? 'Apple 支付' : '微信支付'
}
function openRefundDialog(order) {
  if (!isRefundableOrder(order)) {
    ElMessage.warning(isApplePayOrder(order)
      ? 'Apple 支付不支持开发者主动退款，请引导用户在 App Store 申请退款'
      : '赠送 / 邀请 / 零元订单不能走微信退款')
    return
  }
...
async function onSavePrices() {
  ...
  const invalid = changed.find((it) => !Number.isInteger(it.editPrice) || it.editPrice < 1)
  if (invalid) {
    ElMessage.error((invalid.name || invalid.id) + ' 价格无效')
    return
  }
  const lines = changed
  ...
  try {
    await ElMessageBox.confirm(
      `<div style="line-height:1.7;">
         ...
         <div style="background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.88);padding:8px 10px;border-radius:4px;border:1px solid rgba(255,255,255,0.08);">${lines}</div>
       </div>`,
      '价格变更确认',
      {
...
async function refreshAll() {
  if (whitelistDirty.value) {
    try {
      await ElMessageBox.confirm(
        '白名单有未保存修改，刷新后会丢失。确定刷新？',
        '刷新确认',
        { confirmButtonText: '仍要刷新', cancelButtonText: '取消', type: 'warning' }
      )
    } catch (e) {
      return
    }
  }
  await Promise.all([load(), loadSkuPrices()])
}
</script>
...
.pro-card :deep(.pro-group-header-row) {
  cursor: pointer;
}

.pro-card :deep(.pro-group-header-row td) {
  background: rgba(240, 192, 96, 0.10) !important;
  border-bottom: 1px solid rgba(240, 192, 96, 0.18);
}

.pro-card :deep(.pro-group-header-row:hover td) {
  background: rgba(240, 192, 96, 0.16) !important;
}

.pro-card :deep(.el-table__row.pro-group-header-row .cell) {
  padding-top: 10px;
  padding-bottom: 10px;
}
